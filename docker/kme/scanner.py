...
class Scanner:
    ...
    def start(self) -> None:
        while not self.stop.is_set():
            ...
            # Fetch peer status without holding kme_lock. Holding it during
            # requests.get() deadlocks when both KMEs scan each other: each
            # scanner blocks the peer's HTTP handler in before_request().
            fetched: list[dict] = []
