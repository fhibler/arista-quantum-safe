...
def cleartext_capture_filter(switch_ip: str) -> str:
    ...
    return f"host {switch_ip} and (({udp_ports}) or ({tcp_ports}))"
...
def probe_syslog_tls_pqc(
    ...
    *,
    ...
    command = (
        ...
        f"openssl s_client -connect {syslog_ip}:{SYSLOG_PORT} -servername syslog -tls1_3 "
...
def probe_syslog_delivery_no_cleartext(
    ...
    *,
    ...
    docker_exec(syslog_container, "rm -f /tmp/cleartext-syslog.chk", check=False)
    docker_exec(
        syslog_container,
        f"(timeout 30 tcpdump -i eth0 -n -c 1 -Z root '{capture_filter}' 2>/dev/null "
        f"&& echo CAPTURED > /tmp/cleartext-syslog.chk) &",
        check=False,
    )
    time.sleep(1)
    send_log()
    ...
    while time.time() < deadline:
        captured = docker_exec(
            syslog_container,
            "test -f /tmp/cleartext-syslog.chk && echo yes || echo no",
            check=False,
        )
