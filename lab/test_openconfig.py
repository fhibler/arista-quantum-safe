...
def run_openconfig_checks(
    *,
    ...
    print_test_header(
        "OpenConfig verification (TLS 1.3, PQC-hybrid only — no classical fallback)",
        "  [config] EOS show commands for gNMI, gRIBI, gNSI, gNPSI, RESTCONF, eos-sdk-rpc",
        "  [live / test-runner]  gNMI/gNOI/gRIBI/gNSI/gNPSI gRPC, RESTCONF HTTPS, eos-sdk-rpc mTLS",
    ...
    for index, node in enumerate(CEOS_NODES):
        ...
        for family in IP_FAMILIES:
            ...
            mtls_wire = probe_gnmi_mtls(targets, node, family=family, verbose=verbose)
            probe_gnmi_get(targets, node, family=family, tls_wire=mtls_wire, verbose=verbose)
