...
def ceos_cli(container: str, commands: str) -> str:
    """Run privileged EOS CLI commands."""
    result = docker_exec(container, "Cli", input_text=commands)
...
def openssl_s_client(
    ...
    *,
    ...
    result = docker_exec(radius_container, command, check=False)
...
def check_radius_config(targets: LabTargets) -> None:
    output = docker_exec(
...
def check_switch_ssl_profile(targets: LabTargets, node: str, profile: str) -> None:
...
def check_eapi_config(targets: LabTargets, node: str) -> None:
    container = targets.ceos_container(node)
    check_switch_ssl_profile(targets, node, "EAPI")
...
def check_ssh_pqc_config(targets: LabTargets, node: str) -> None:
    container = targets.ceos_container(node)
...
def check_radsec_config(targets: LabTargets, node: str) -> None:
    container = targets.ceos_container(node)
    check_switch_ssl_profile(targets, node, "RADSEC")
...
def probe_eapi_https(targets: LabTargets, node: str) -> None:
...
def probe_eapi_jsonrpc(node: str, switch_ip: str) -> None:
    payload = (
...
def probe_ssh_pqc(targets: LabTargets, node: str, peer: str) -> None:
    ...
    result = docker_exec(container, command, check=False)
    output = result.stdout + result.stderr
...
def probe_radsec_from_switch(targets: LabTargets, node: str) -> None:
    container = targets.ceos_container(node)
    output = ceos_cli(
        container,
        "enable\n"
...
def run_live_checks(
    *,
    ...
    print_device("radius")
    if not skip_config:
        check_radius_config(targets)

    for node in ("ceos1", "ceos2"):
        print()
        print_device(node)
        if not skip_config:
            check_eapi_config(targets, node)
            check_radsec_config(targets, node)
            check_ssh_pqc_config(targets, node)
        probe_eapi_https(targets, node)
        probe_eapi_jsonrpc(node, targets.ceos_ips[node])
        probe_radsec_from_switch(targets, node)
        probe_ssh_pqc(targets, node, CEOS_PEERS[node])
