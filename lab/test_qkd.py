...
def check_rotation_success_logs(container: str, node: str, *, verbose: bool | None) -> str:
    output = ceos_cli(container, "enable\nshow logging 500\n", verbose=verbose)
    for line in output.splitlines():
        if any(marker in line for marker in ROTATION_SUCCESS_MARKERS):
            report_log(f"found rotation success: {line.strip()}")
            return line.strip()
    raise QkdCheckError(
        f"{node}: no QUADRA rotation success message in recent syslog "
        "(expected %QUADRA-4-ROTATION_SUCCESS%)"
    )
...
def run_qkd_checks(
    *,
    ...
    print("  [log]   %QUADRA-4-ROTATION_SUCCESS% in recent syslog")
    ...
    check_rotation_success_logs(master_container, MASTER, verbose=verbose)
    ...
    check_rotation_success_logs(slave_container, SLAVE, verbose=verbose)
    ...
    report_summary(
        "QuaDRA",
        "daemon master/slave healthy, rotation success logged, KME enc_keys/dec_keys seen, next rotation scheduled",
    )
    return True
